...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
        ...
        # Force zero‑padding (no reflect) for both extractors
    ...
    def reshape_wav2img(self, x):
        B, C, T, F = x.shape
        target_T = int(self.spec_size * self.freq_ratio)  # e.g., 256*4 = 1024
        target_F = self.spec_size // self.freq_ratio  # e.g., 256//4 = 64

        # If T or F exceed target dimensions, crop them.
        if T > target_T:
            x = x[:, :, :target_T, :]
        if F > target_F:
            x = x[:, :, :, :target_F]

        # If they are smaller than target, interpolate (as before)
        if x.shape[2] < target_T:
            x = torch.nn.functional.interpolate(x, (target_T, x.shape[3]), mode="bicubic", align_corners=True)
        if x.shape[3] < target_F:
            x = torch.nn.functional.interpolate(x, (x.shape[2], target_F), mode="bicubic", align_corners=True)
    ...
    # Modification performed here

    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):
        # x is expected to have shape [B, 3, L] where L is the number of time samples.
        B, C, L = x.shape  # Here, C should be 3.

        # --- Process each channel independently using the spectrogram and log-mel extractors ---
        # Reshape to process each channel as an independent example:
        x = x.view(B * C, L)  # Now shape: [B*3, L]

        # Compute spectrogram: expects input shape [batch, data_length]
        x = self.spectrogram_extractor(x)
        # Expected output shape: [B*3, 1, T, F_spec]

        # Compute log-mel features
        x = self.logmel_extractor(x)
        # Expected output shape: [B*3, 1, T, mel_bins]

        # --- Reshape back to recover the original channel structure ---
        _, _, T, mel_bins = x.shape
        # Reshape from [B*3, 1, T, mel_bins] to [B, 3, 1, T, mel_bins]
        x = x.view(B, C, 1, T, mel_bins)
        # Squeeze the singleton dimension so that x becomes [B, 3, T, mel_bins]
        x = x.squeeze(2)

        # --- Continue with the rest of the forward pass ---
        # The original code transposes to apply batch normalization.
        # Here, we follow the same procedure.
        x = x.transpose(1, 3)  # Now shape: [B, mel_bins, T, 3]
        x = self.bn0(x)
        x = x.transpose(1, 3)  # Back to shape: [B, 3, T, mel_bins]
        ...
        # The remaining branch logic (infer_mode, repeat_mode, cropping) remains unchanged.
        if infer_mode:
            frame_num = x.shape[2]
            target_T = int(self.spec_size * self.freq_ratio)
            ...
            else:  # typical, most straightforward case
                x = self.reshape_wav2img(x)
                output_dict = self.forward_features(x)
        return output_dict
    ...
    #     # Reshape so that each channel is processed separately.
    #     x = x.view(B * C, L)  # New shape: [B*3, L]
    #
    #     # Compute spectrogram and log-mel features for each channel.
    #     # These modules expect 2D input: [batch, data_length]
    #     x = self.spectrogram_extractor(x)  # Expected output shape: [B*3, 1, T, F_spec]
    #     x = self.logmel_extractor(x)  # Expected output shape: [B*3, 1, T, mel_bins]
    #
    #     # Reshape to recover the channel dimension.
    #     _, _, T, mel_bins = x.shape
    #     x = x.view(B, C, 1, T, mel_bins)  # Now shape: [B, 3, 1, T, mel_bins]
    #     x = x.squeeze(2)  # Now shape: [B, 3, T, mel_bins]
    #
    #     # --- Continue with the original processing ---
    #     # The original code transposes to move the mel_bins dimension into the channel position,
    #     # applies batch normalization (bn0), then transposes back.
    #     x = x.transpose(1, 3)  # New shape: [B, mel_bins, T, 3]
    #     x = self.bn0(x)
    #     x = x.transpose(1, 3)  # Now back to [B, 3, T, mel_bins]
    #
    #     if self.training:
    #         x = self.spec_augmenter(x)
    #     if self.training and mixup_lambda is not None:
    #         x = do_mixup(x, mixup_lambda)
    #
    #     # The remaining processing (handling infer_mode, repeat mode, cropping, etc.) remains unchanged.
    #     if infer_mode:
    #         frame_num = x.shape[2]
    #         target_T = int(self.spec_size * self.freq_ratio)
    #         repeat_ratio = math.floor(target_T / frame_num)
    ...
    # def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False):
    #     B, C, T = x.shape
    #     x = x.view(B * C, 1, T)  # ← ensure 3‑D (batch*channels, 1, time)
    #     print("Input to spectrogram_extractor:", x.shape)
    #     x = self.spectrogram_extractor(x)
    #     x = self.logmel_extractor(x)
    #     _, _, time, mel = x.shape
    #     x = x.view(B, C, time, mel)
    #
    #     x = x.transpose(1, 3)
    #     x = self.bn0(x)
    #     x = x.transpose(1, 3)
    ...
    #             output_dicts = []
    #             for cur_pos in range(0, (self.freq_ratio - 1) * self.spec_size + 1, self.spec_size):
    #                 tx = self.repeat_wat2img(x.clone(), cur_pos)
    #                 output_dicts.append(self.forward_features(tx))
    ...
    #                 crop_size = (x.shape[2] - 1) // 2
    #                 outputs = []
    #                 for cur_pos in range(0, x.shape[2] - crop_size - 1, overlap):
    #                     tx = self.crop_wav(x, crop_size=crop_size, spe_pos=cur_pos)
    #                     tx = self.reshape_wav2img(tx)
    #                     outputs.append(self.forward_features(tx))
    #                 clipwise_output = sum(d["clipwise_output"] for d in outputs) / len(outputs)
    #                 framewise_output = sum(d["framewise_output"] for d in outputs) / len(outputs)
    #                 output_dict = {'framewise_output': framewise_output, 'clipwise_output': clipwise_output}
    #         else:
    #             x = self.reshape_wav2img(x)
    #             output_dict = self.forward_features(x)
